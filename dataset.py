...
def main():
    ...
    import torchvision.transforms as transforms
    # file_root = '/data/VOC2012/allimgs/'
    # file_root = '../data/VOC2012trainval/JPEGImages/'
    file_root = '../data/VOC2007test/JPEGImages/'
    # 构建数据集
    train_dataset = yoloDataset(
        root=file_root, list_file='../data/voc2012trainval.txt', train=True, transform=[transforms.ToTensor()])
    validation_dataset = yoloDataset(
        root=file_root, list_file='../data/voc2007test.txt', train=False, transform=[transforms.ToTensor()])
    # 构建加载器
    ...
    for i in range(1):
        ...
        img, target = next(validation_iter)
        print(img.shape)
